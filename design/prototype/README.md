...
- **Boarding-pass cards**: each segment is a compact three-column card. Row 1 shows
  ...
  departure time, the trip duration, and the arrival time. Cards are quiet by default;
  **hovering or selecting** one reveals compact action buttons as a pill pinned to the
  bottom-right corner without changing the card's size, and truncated fields show their
