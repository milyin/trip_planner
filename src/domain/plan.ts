import type { Segment, TripItem } from './types';
...
export const planItems = (items: TripItem[]): TripItem[] =>
  items.filter((r) => r.inPlan).sort((a, b) => startMs(a) - startMs(b));

/** Not-yet-planned items ordered chronologically (the Segments panel). */
export const listItems = (items: TripItem[]): TripItem[] =>
  items.filter((r) => !r.inPlan).sort((a, b) => startMs(a) - startMs(b));

/** The plan segment a not-yet-planned segment time-overlaps (or `null`). */
export function conflictOf(items: TripItem[], r: TripItem): Segment | null {
  if (r.kind !== 'segment' || r.inPlan) return null;
  const hit = planItems(items).find((p) => p.kind === 'segment' && overlaps(r, p));
  return (hit as Segment) ?? null;
}
...
export function classifyGap(prev: TripItem, next: TripItem): GapInfo {
  const availMin = (startMs(next) - endMs(prev)) / 60000;
  const need = next.kind === 'segment' ? bufferMin(next.transport) : 0;
...
export function gapNights(prev: TripItem, next: TripItem): number {
...
export function gapRemote(prev: TripItem, next: TripItem): boolean {
...
export function planTotals(plan: TripItem[]): PlanTotals {
